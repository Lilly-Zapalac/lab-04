...
## the p-value is smaller than the 0.05 threshold and therefore it is saying that you would reject the null hypothesis and therefore there is a relationship between the channel type and forest type in salamander abundance.
```
...
```{r}
?and_vertebrates
body_length <- trout_clean |> 
  filter(section == "CC") |> 
  pull(length_1_mm)

body_mass <- trout_clean |> 
  filter(section == "OG") |> 
  pull(weight_g)

var.test(body_length, body_mass)

t.test(log(sally_clean$weight_g) ~ sally_clean$section, var.equal = TRUE)

## We would reject the null because the p-value is below the 0.05 threshold. Therefore, there is not a significant difference in species biomass between clear cut and old growth forests.
...
## There is an almost perfect correlation between the length and weight of the trout since the rho value is almost 1. 
```

```{r}
ggpubr::ggarrange(ggpubr::gghistogram(body_length, main = "body length"), 
                  ggpubr::gghistogram(body_mass, main = "Body Mass"))

## these graphs show that the data is not distributed normally, the are rather left skewed.
```
...
## The findings in this boxplot is that when the latitude increases the size of the crabs also increases overall as well.
```
...
```{r}
library(ggpubr)
pie_lm <- lm(water_temp_sd ~ size, data = crab_data_sorted)

summary(crab_data_sorted)
...
## From the scatter plot we can tell that there is not really a correlation between water_temp_sd and the size of the crabs.
```
...
pie_crab |> 
  ...
  cor()
## It is found that air_temp_sd and latitude have a high correlation coefficient meaning they are highly correlated. While, water_temp_sd is not highly correlated with anything since the correlation coefficient is .4.
```
```{r}
pie_multiple_regression <- lm(size ~ latitude + air_temp_sd + water_temp_sd, data = pie_crab)

summary(pie_multiple_regression)
## The p-value is small meaning we will reject the null hypothesis and therefore this data does not fit a normal distribution. The R^2 is also small meaning there is a small correlation between the variables.
```
